<template>
  <div class="app-container">
      ...
      <el-table-column :label="$t('common.operation')" align="center" class-name="small-padding fixed-width" width="220">
        <template slot-scope="scope">
          <el-button
            ...
            v-hasPermi="['oa:attendanceLeave:edit']"
            v-if="scope.row.status === 'draft'"
          >{{ $t('oa.attendance.submitLeave') }}</el-button>
          <el-button
            size="mini"
            type="text"
            icon="el-icon-edit"
          ...
          <el-button
            ...
            v-hasPermi="['oa:attendanceLeave:remove']"
    ...
    <flow-submit-dialog
      ...
      :business-key="currentRow ? 'attendance_leave:' + currentRow.id : ''"
...
<script>
import { listLeave, delLeave, submitLeave } from "@/api/oa/attendance"
...
export default {
  ...
  methods: {
    ...
    handleAdd() {
      this.$router.push('/oa/attendance/leave/form?mode=add')
    },
    handleUpdate(row) {
      const id = row ? row.id : this.ids[0]
      this.$router.push('/oa/attendance/leave/form?mode=edit&id=' + id)
    },
    handleSubmit(row) {
      this.currentRow = row
    ...
    submitApi(data) {
      return submitLeave(this.currentRow.id, data)
    },
    handleSubmitSuccess() {
      this.getList()
