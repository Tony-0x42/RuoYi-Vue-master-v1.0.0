<template>
  <oa-form-page @submit="handleSubmit" @save="handleSave" @close="handleClose">
        ...
        <el-row>
          ...
          <el-col :span="12">
            <el-form-item :label="$t('oa.attendance.destination')" prop="destination">
              <el-input v-model="form.destination" :placeholder="$t('oa.attendance.placeholder.destination')" :disabled="isDetail" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
    ...
    <flow-submit-dialog
      ...
      :business-key="'attendance_trip:' + form.id"
...
import { getTrip, addTrip, updateTrip, submitTrip } from "@/api/oa/attendance"
  ...
  methods: {
    ...
    handleSubmit() {
      this.$refs["form"].validate(valid => {
        ...
        if (!this.form.id) {
          this.$modal.msgWarning("请先保存出差申请")
          return
        }
    ...
    submitApi(data) {
      return submitTrip(this.form.id, data)
    },
    handleClose() {
      this.$router.back()
    }
